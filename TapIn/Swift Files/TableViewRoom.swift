...
class ServerRoomViewModel: ObservableObject {
    ...
    func listenToFloorPlan() {
        floorplanListener = db.collection("settings").document("floorplan").addSnapshotListener { snap, _ in
            guard let data = snap?.data(),
                ...
                // ✅ RESTORED ORIGINAL LAYOUT AS DEFAULT!
    ...
    func markAsServed(_ id: String) {
        db.collection("kitchenOrders").document(id).updateData(["status": "served"])
    }
    
    // ✅ The exact classic layout you had before, saved here to prevent blank screens!
...
struct TableViewRoom: View {
    ...
    var body: some View {
        ...
        ZStack(alignment: .leading) {
            ...
            if showPanel || showNotifications {
            ...
            if showPanel, let table = selectedTable {
                SidePanel(
                    ...
                    close: { withAnimation(.spring()) { showPanel = false } },
    ...
    // Safety check in case a room gets deleted while viewing
    var safeIndex: Int {
    ...
    func tableButton(_ label: String, _ shape: TableShape, _ x: CGFloat, _ y: CGFloat) -> some View {

        let table = vm.tables.first(where: { $0.tableNumber == label })
        let occupied = (table?.items.contains(where: { $0.qty > 0 }) ?? false)
        let isReady = serverVM.readyNotifications.contains { $0.tableNumber == label }
        ...
        return TableView(label: label, shape: shape, color: color) {
            if let info = table {
                ...
                selectedTable = TableInfo(tableNumber: label, guests: 0, items: [])
            }
    ...
    func updateGuestCount(table: TableInfo, guests: Int) {
        db.collection("tables").document(table.tableNumber).updateData(["guests": guests])
    }
}
