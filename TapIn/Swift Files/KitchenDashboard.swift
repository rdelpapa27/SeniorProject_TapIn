...
final class KitchenDashboardViewModel: ObservableObject {
    ...
    func listen() {
        listener = db.collection("kitchenOrders")
            ...
            .addSnapshotListener { snap, error in
                ...
                // ✅ Catches the missing index error and prints it loudly so you can click the link!
                if let error = error {
                    print("🚨 KITCHEN LISTENER ERROR: \(error.localizedDescription)")
                    return
                }
                
                guard let docs = snap?.documents else { return }
                
                let updatedOrders = docs.compactMap { self.map($0) }
                
                // ✅ Forces the iPad to redraw the screen the millisecond new data arrives
    ...
    func clear(_ order: KitchenOrder) {
        // Remove locally immediately for a snappy UI
        DispatchQueue.main.async {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack {
                    Text("Order #\(String(order.orderNumber.prefix(4)))")
